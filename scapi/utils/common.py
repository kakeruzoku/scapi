...
class UnknownDict(dict[_KT, _VT]):
    ...
    def get(
        self, key: _KT, default: UNKNOWN_TYPE = UNKNOWN, /
    ) -> _VT | UNKNOWN_TYPE: ...
...
@overload
def split(
    text: str, before: str, after: str, forced: Literal[False] = False
) -> str | None: ...
...
async def get_any_count(
    client: HTTPClient, url: str, before: str, after: str = ")"
) -> int:
...
def b62decode(text: str):
    ...
    return sum(
        [
            BASE62_ALPHABET.index(text[i]) * (62 ** (text_len - i - 1))
...
@overload
def dt_from_isoformat(
    timestamp: str | _T, allow_unknown: Literal[True]
) -> datetime.datetime | _T: ...


@overload
def dt_from_isoformat(
    timestamp: str | Any, allow_unknown: Literal[False]
) -> datetime.datetime: ...


def dt_from_isoformat(
    timestamp: str | _T, allow_unknown: bool = True
) -> datetime.datetime | _T:
    if not isinstance(timestamp, str):
        if allow_unknown:
            ...
            raise ValueError()
    return datetime.datetime.fromisoformat(timestamp).replace(
...
def dt_from_timestamp(
    timestamp: float | _T, allow_unknown: Literal[True]
) -> datetime.datetime | _T: ...


@overload
def dt_from_timestamp(
    timestamp: float | Any, allow_unknown: Literal[False]
) -> datetime.datetime: ...


def dt_from_timestamp(
    timestamp: float | _T, allow_unknown: bool = True
) -> datetime.datetime | _T:
...
def _count_func(
    ...
    **kwargs,
    ...
    if is_page:
        ...
        async def _func(c: int, max_limit: int) -> int:
            return len(
                [i async for i in func(max_limit, (c - 1) * max_limit, *args, **kwargs)]
            )
...
async def maybe_coroutine(
    func: Callable[_P, Coroutine[Any, Any, _T] | _T],
    *args: _P.args,
    **kwargs: _P.kwargs,
) -> _T:
