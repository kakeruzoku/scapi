...
class Project(_BaseSiteAPI[int]):
    ...
    def _update_from_data(self, data: ProjectPayload):
        ...
        if _author:
            if self.author is UNKNOWN:
                ...
                self.author = User(
                    _author.get("username"), self.client_or_session, is_real=True
                )
        ...
        if _remix:
            self._update_to_attributes(
                remix_parent_id=_remix.get("parent"), remix_root_id=_remix.get("root")
            )
    ...
    @staticmethod
    def _get_time_from_remixtree(
        data: MAYBE_UNKNOWN[RemixTreeDatetimePayload | None],
    ) -> str | UNKNOWN_TYPE:
        if data is UNKNOWN:
            return UNKNOWN
        if data is None:
    ...
    def _update_from_remixtree(self, data: RemixTreePayload):
        self._update_to_attributes(
            ...
            _modified_at=self._get_time_from_remixtree(
                data.get("datetime_modified") or data.get("mtime")
            ),
    ...
    def thumbnail_url(self) -> str:
        ...
        """
        return (
            f"https://uploads.scratch.mit.edu/get_image/project/{self.id}_480x360.png"
        )
    ...
    async def get_studios(
        ...
        async for _s in api_iterative(
            self.client,
            f"https://api.scratch.mit.edu/users/{self._author_username}/projects/{self.id}/studios",
            limit=limit,
            offset=offset,
        ):
    ...
    async def get_parent_project(self) -> "Project|None|UNKNOWN_TYPE":
        ...
        if isinstance(self.remix_parent_id, int):
            return await self._create_from_api(
                self.remix_parent_id, self.client_or_session
            )
    ...
    async def get_root_project(self) -> "Project|None|UNKNOWN_TYPE":
        ...
        if isinstance(self.remix_root_id, int):
            return await self._create_from_api(
                self.remix_root_id, self.client_or_session
            )
    ...
    async def get_comments(
        ...
        async for _c in api_iterative(
            self.client,
            f"https://api.scratch.mit.edu/users/{self._author_username}/projects/{self.id}/comments",
            limit=limit,
            offset=offset,
        ):
    ...
    async def edit(
        ...
        *,
        ...
        if comment_allowed is not None:
            data["comment_allowed"] = comment_allowed
        ...
        r = await self.client.put(
            f"https://api.scratch.mit.edu/projects/{self.id}", json=data
        )
    ...
    async def old_edit(
        ...
        *,
        ...
        if share is not None:
            data["isPublished"] = share
        if title is not None:
            data["title"] = title
        if trash is not None:
            data["visibility"] = "trshbyusr" if trash else "visible"
        r = await self.client.put(
            f"https://scratch.mit.edu/site-api/projects/all/{self.id}/", json=data
        )
        _data: OldProjectEditPayload = r.json()
        self._update_to_attributes(
            title=_data.get("title"), _modified_at=_data.get("datetime_modified")
        )
    ...
    async def get_visibility(self) -> "ProjectVisibility":
        ...
        self.require_session()
        response = await self.client.get(
            f"https://api.scratch.mit.edu/users/{self._session.username}/projects/{self.id}/visibility"
        )
    ...
    async def is_loved(self) -> bool:
        ...
        self.require_session()
        response = await self.client.get(
            f"https://api.scratch.mit.edu/projects/{self.id}/loves/user/{self._session.username}"
        )
    ...
    async def is_favorited(self) -> bool:
        ...
        self.require_session()
        response = await self.client.get(
            f"https://api.scratch.mit.edu/projects/{self.id}/favorites/user/{self._session.username}"
        )
    ...
    async def add_view(self) -> bool:
        ...
        try:
            await self.client.post(
                f"https://api.scratch.mit.edu/users/{self._author_username}/projects/{self.id}/views/"
            )
    ...
    async def report(self, category: int, message: str) -> str:
        ...
        response = await self.client.post(
            ...
            json={"notes": message, "report_category": str(category), "thumbnail": ""},
        )
...
class ProjectFeatured:
    ...
    def __init__(self, data: UserFeaturedPayload, user: "User"):
        ...
        self.label: ProjectFeaturedLabel = ProjectFeaturedLabel.get_from_id(
            data.get("featured_project_label_id")
        )
    ...
    def _update_from_data(self, data: RemixTreePayload):
        if self.project.author is UNKNOWN:
            ...
            self.project.author = User(
                data["username"], self.client_or_session, is_real=True
            )
...
def get_project(
    project_id: int, *, _client: HTTPClient | None = None
) -> _AwaitableContextManager[Project]:
    """
    プロジェクトを取得する。
...
async def _get_remixtree(
    project_id: int, *, client_or_session: "Session|HTTPClient|None" = None
) -> RemixTree:
...
def get_remixtree(
    project_id: int, *, client_or_session: "Session|HTTPClient|None" = None
) -> _AwaitableContextManager[RemixTree]:
    """
    リミックスツリーを取得する。
    ...
    return _AwaitableContextManager(
        _get_remixtree(project_id, client_or_session=client_or_session)
    )
