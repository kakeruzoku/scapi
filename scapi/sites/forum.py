...
class ForumCategory(_BaseSiteAPI[int]):
    ...
    async def get_topics(
    ...
    ) -> AsyncGenerator["ForumTopic", None]:
        """
        ...
        for i in range(start_page, end_page + 1):
            response = await self.client.get(
                f"https://scratch.mit.edu/discuss/{self.id}/", params={"page": i}
            )
            ...
            for _topic in _body.find_all("tr"):
                yield ForumTopic._create_from_category(
                    self, _topic, self.client_or_session
                )
...
class ForumTopic(_BaseSiteAPI):
    ...
    def _create_from_category(
        ...
        topic.author = User(
            _user.get_text(strip=True).removeprefix("by "),
            client_or_session,
            is_real=True,
        )
    ...
    async def update(self):
        response = await self.client.get(
            f"https://scratch.mit.edu/discuss/topic/{self.id}/"
        )
        self._update_from_data(bs4.BeautifulSoup(response.text, "html.parser"))
    ...
    def _update_from_data(self, data: Tag):
        ...
        if self.category is UNKNOWN:
            self.category = ForumCategory(
                int(split(str(_category_a["href"]), "/discuss/", "/", True)),
    ...
    async def follow(self):
        ...
        await self.client.post(
            f"https://scratch.mit.edu/discuss/subscription/topic/{self.id}/add/"
        )
    ...
    async def unfollow(self):
        ...
        """
        await self.client.post(
            f"https://scratch.mit.edu/discuss/subscription/topic/{self.id}/remove/"
        )

    async def get_posts(
        self, start_page: int | None = None, end_page: int | None = None
    ) -> AsyncGenerator["ForumPost", None]:
        """
        トピックに投稿された投稿を取得します。
        ...
        for i in range(start_page, end_page + 1):
            response = await self.client.get(
                f"https://scratch.mit.edu/discuss/topic/{self.id}/", params={"page": i}
            )
            data = bs4.BeautifulSoup(fix_html(response.text), "html.parser")
            ...
            if is_first:
                self._update_from_data(data)
                is_first = False
            _posts: bs4.ResultSet[Tag] = data.find_all(
                "div", {"class": "blockpost roweven firstpost"}
            )
            for _post in _posts:
                id = int(str(_post["id"]).removeprefix("p"))
                yield ForumPost._create_from_data(
                    id, _post, self.client_or_session, topic=self
                )
...
class ForumPost(_BaseSiteAPI):
    ...
    async def update(self):
        response = await self.client.get(
            f"https://scratch.mit.edu/discuss/post/{self.id}/"
        )
    ...
    def _update_from_data(self, data: bs4.Tag):
        ...
        if self.topic is UNKNOWN:
            ...
            self.topic = ForumTopic(
                int(split(str(_meta_url["content"]), "/topic/", "/", True)),
                self.client_or_session,
            )
        ...
        if self.author is UNKNOWN:
            self.author = User(
                split(str(_author_a["href"]), "/users/", "/", True),
                self.client_or_session,
                is_real=True,
            )
        ...
        if _edit is None:
            ...
            if (
                not isinstance(self.modified_by, User)
            ) or _edited_by.lower() != self.modified_by.lower_username:
                self.modified_by = User(
                    _edited_by, self.client_or_session, is_real=True
                )
            self.modified_at = decode_datetime(
                split(str(_edit.get_text()), "(", ")", True)
            )
    ...
    async def get_source(self) -> str:
        ...
        """
        response = await self.client.get(
            f"https://scratch.mit.edu/discuss/post/{self.id}/source/"
        )
    ...
    async with temporary_httpclient(client_or_session) as client:
        ...
        for box in boxes.find_all("div", {"class": "box"}):
            ...
            for category in categories:
                returns[box_title].append(
                    ForumCategory._create_from_home(
                        box_title, category, client_or_session or client
                    )
                )
    return returns
...
def get_forum_category(
    category_id: int, *, _client: "HTTPClient|None" = None
) -> _AwaitableContextManager[ForumCategory]:
    """
    フォーラムカテゴリーを取得する。
    ...
    return _AwaitableContextManager(
        ForumCategory._create_from_api(category_id, _client)
    )


def get_forum_topic(
    topic_id: int, *, _client: "HTTPClient|None" = None
) -> _AwaitableContextManager[ForumTopic]:
    """
    フォーラムトピックを取得する。
...
def get_forum_post(
    post_id: int, *, _client: "HTTPClient|None" = None
) -> _AwaitableContextManager[ForumPost]:
    """
    フォーラムの投稿を取得する。
...
def load_last_post(self: _BaseSiteAPI, data: bs4.Tag) -> ForumPost:
    ...
    post = ForumPost(
        int(split(_last_post_url, "/discuss/post/", "/", True)), self.client_or_session
    )
    post.author = User(
        _post_author.get_text(strip=True).removeprefix("by "),
        self.client_or_session,
        is_real=True,
    )
